...
const {
  ...
  addgroupdata,
  removeuser,
  remainingmembersingroup,
  printgroupsarray,
  pushmessagetogroup,
  getmessagesfromgroup,
  cleanmessagesfromgroup,
} = require('./helper.js');
...
app.post('/createGroup', async (req, res) => {
  try {
    var searchedGroup = await Group.findOne({
    ...
    if (!searchedGroup) {
      ...
      return res
        .status(200)
        .json({
          data: { message: 'Group Created', groupdata: newGroup, user: user },
        });
    }
...
app.post('/joinGroup', async (req, res) => {
  try {
    ...
    if (group) {
      var user = await User.findById(req.user._id);
      var GroupPresentInUser = user.groups.find(
        (hisgroup) => hisgroup.toString() === group._id.toString()
      );
      if (GroupPresentInUser) {
        console.log('Group already present in user');
        return res.status(500).send('Group already present in User!!');
      }
      var userPresentInGroup = group.members.find(
        (userdata) => userdata.id === req.user._id.toString()
      );
      if (userPresentInGroup) {
        console.log('User already Present In Members Array Of  This Group');
        return res
          .status(500)
          .send('User already Present In Members Array Of  This Group');
      }
      ...
      return res
        .status(200)
        .json({
          data: { message: 'Joining Successful', groupdata: group, user: user },
        });
    }
...
io.on('connection', (socket) => {
    ...
    if (returnedusers) {
      socket.broadcast
        ...
        .emit('notification', {
          notification: {
            room: data.room,
            text: `${data.name} has Joined to room ${data.room}`,
          },
        });
  ...
  socket.on('sendmessage', (data, callback) => {
    ...
    console.log(
      'message has successfully pushed to group',
      pushmessagetogroup(data)
    );
  ...
  socket.on('disconnect', async function () {
    ...
    if (data.length > 0) {
      for (var i = 0; i < data.length; i++) {
        console.log(
          `remaining members in ${data[i].room} is:`,
          remainingmembersingroup(data[i].room)
        );
        ...
        socket.broadcast
          ...
          .emit('notification', {
            notification: {
              room: data[i].room,
              text: `${data[i].name} disconnected from ${data[i].room}`,
            },
          });
